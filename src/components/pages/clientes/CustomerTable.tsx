...
import { maskDate, maskCpf, maskPhone } from "@/utils/mask.util";
import { TCustomer, ResetCustomer } from "@/types/domain.type";
...
export default function CustomerTable() {
  ...
  const [customer, setCustomer] = useState<TCustomer>(ResetCustomer);
  ...
  const getAll = async (page: number) => {
    try {
      ...
      const { data } = await api.get(
        `/customers?deleted=false&orderBy=createdAt&sort=desc&pageSize=10&pageNumber=${page}`,
        configApi()
      );
      const result = (data as any).result;
      setPagination({ currentPage: result.currentPage, data: result.data, sizePage: result.pageSize, totalPages: result.totalPages, totalCount: result.totalCount });
  ...
  const destroy = async () => {
    try {
      ...
      resolveResponse({ status: 204, message: "Cliente excluído com sucesso" });
      closeModal();
      await getAll(1);
    } catch (error) { resolveResponse(error); }
    finally { setLoading(false); }
  ...
  const getObj = (obj: any, action: string) => {
    setCustomer(obj);
    if (action === "edit") router.push(`/clientes/${obj.id}`);
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-4">
        ...
        <Link href="/clientes/novo" className="btn-erp-primary bg-brand-500 hover:bg-brand-600 w-auto px-5 gap-2">
      ...
      {pagination.data.length > 0 ? (
        <>
                  ...
                  <TableBody className="divide-y divide-gray-100 dark:divide-white/5">
                    {pagination.data.map((x: any) => (
                      <TableRow key={x.id}>
                        ...
                        <TableCell className="px-5 py-4 text-start text-gray-500 dark:text-gray-400">{x.cpf ? maskCpf(x.cpf) : "-"}</TableCell>
          ...
          <Pagination currentPage={pagination.currentPage} totalCount={pagination.totalCount} totalData={pagination.data.length} totalPages={pagination.totalPages} onPageChange={(page) => getAll(page)} />
