...
const AppSidebar: React.FC = () => {
  ...
  const getAuthorizedMenu = useCallback(() => {
    ...
    const modulesStr = localStorage.getItem("pulpaModules");
    const masterStr = localStorage.getItem("pulpaMaster");
  ...
  return (
    <aside 
      ...
      <div className={`py-8 flex ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>
        <Link href={`${isAdmin ? '/pdv' : '/master-data/profile'}`} className="w-full flex justify-center">
